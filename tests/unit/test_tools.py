...
class TestToolLoading:
    ...
    def test_tool_loading_basic(self, mock_youtrack_client):
        ...
        with patch('youtrack_mcp.tools.issues.IssueTools', return_value=mock_instance):
            with patch('youtrack_mcp.tools.projects.ProjectTools', return_value=mock_instance):
                with patch('youtrack_mcp.tools.users.UserTools', return_value=mock_instance):
                    with patch('youtrack_mcp.tools.search.SearchTools', return_value=mock_instance):
                        with patch('youtrack_mcp.tools.resources.ResourcesTools', return_value=mock_instance):
    ...
    def test_tool_definitions_integration(self, mock_youtrack_client):
        ...
        with patch('youtrack_mcp.tools.issues.IssueTools', return_value=mock_instance):
            with patch('youtrack_mcp.tools.projects.ProjectTools', return_value=Mock(get_tool_definitions=Mock(return_value={}))):
                with patch('youtrack_mcp.tools.users.UserTools', return_value=Mock(get_tool_definitions=Mock(return_value={}))):
                    with patch('youtrack_mcp.tools.search.SearchTools', return_value=Mock(get_tool_definitions=Mock(return_value={}))):
                        with patch('youtrack_mcp.tools.resources.ResourcesTools', return_value=Mock(get_tool_definitions=Mock(return_value={}))):
                            tools = load_all_tools()
    ...
    def test_loader_handles_empty_tools(self, mock_youtrack_client):
        ...
        with patch('youtrack_mcp.tools.issues.IssueTools', return_value=empty_mock):
            with patch('youtrack_mcp.tools.projects.ProjectTools', return_value=empty_mock):
                with patch('youtrack_mcp.tools.users.UserTools', return_value=empty_mock):
                    with patch('youtrack_mcp.tools.search.SearchTools', return_value=empty_mock):
                        with patch('youtrack_mcp.tools.resources.ResourcesTools', return_value=empty_mock):
                            tools = load_all_tools()
                            
                            # Should have tools loaded (at least issue_create_issue)
                            assert isinstance(tools, dict)
                            assert len(tools) >= 1  # At least the issue_create_issue tool
