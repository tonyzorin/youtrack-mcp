...
from youtrack_mcp.api.client import (
    YouTrackClient, YouTrackModel, YouTrackAPIError, RateLimitError,
    ResourceNotFoundError, AuthenticationError, PermissionDeniedError,
...
class TestYouTrackClient:
    ...
    def test_handle_response_400_validation_error(self, client):
        ...
        with pytest.raises(ValidationError, match="API request failed with status 400: Invalid request"):
    ...
    def test_handle_response_401_auth_error(self, client):
        ...
        with pytest.raises(AuthenticationError, match="API request failed with status 401: Unauthorized"):
            client._handle_response(response)
    ...
    def test_handle_response_403_permission_error(self, client):
        ...
        with pytest.raises(PermissionDeniedError, match="API request failed with status 403: Permission denied"):
            client._handle_response(response)
    ...
    def test_handle_response_404_not_found(self, client):
        ...
        with pytest.raises(ResourceNotFoundError, match="API request failed with status 404: Resource not found"):
            client._handle_response(response)
    ...
    def test_handle_response_429_rate_limit(self, client):
        ...
        with pytest.raises(RateLimitError, match="API request failed with status 429: Rate limit exceeded"):
            client._handle_response(response)
    ...
    def test_handle_response_unknown_error(self, client):
        ...
        with pytest.raises(YouTrackAPIError, match="API request failed with status 418: Unknown error"):
            client._handle_response(response)
    ...
    def test_make_request_success(self, client, mock_session):
        ...
        mock_session.request.assert_called_once_with("GET", "https://test.youtrack.cloud/api/issues")
        assert result == {"test": "data"}
    ...
    def test_put_method(self, client, mock_session):
        ...
        mock_session.request.assert_called_once_with(
            "PUT", "https://test.youtrack.cloud/api/issues/ISSUE-1", data=data, json=None
        )
    ...
    def test_ssl_verification_disabled(self, mock_session):
        ...
        with patch('youtrack_mcp.api.client.config') as mock_config:
            ...
            with patch('youtrack_mcp.api.client.requests.packages.urllib3.disable_warnings') as mock_disable:
                client = YouTrackClient()
