...
def load_all_tools() -> Dict[str, Callable]:
    ...
    for tool_class in tool_classes:
        ...
        if hasattr(tool_class, 'get_tool_definitions') and callable(getattr(tool_class, 'get_tool_definitions')):
            ...
            for tool_name, definition in class_tool_defs.items():
                if tool_name not in all_tool_definitions:
                    ...
                    current_class = all_tool_definitions[tool_name].get('source_class', '')
                    current_priority = TOOL_PRIORITY.get(current_class, {}).get(tool_name, 10) 
                    new_priority = TOOL_PRIORITY.get(class_name, {}).get(tool_name, 10)
                    
                    if new_priority > current_priority:
                        definition['source_class'] = class_name
                        all_tool_definitions[tool_name] = definition
                        logger.debug(f"Using tool definition for '{tool_name}' from {class_name} (higher priority)")
    ...
    for tool_class in tool_classes:
        ...
        for name, method in class_tools.items():
            ...
            priority = TOOL_PRIORITY.get(class_name, {}).get(name, 10)  # Default priority is 10
    ...
    for name, sources in tool_sources.items():
        if len(sources) > 1:
            logger.warning(f"Tool {name} exists in multiple classes: {', '.join(sources)}")
            # Get the highest priority source
            highest_priority_source = max(tool_priorities[name], key=lambda x: x[1])
            logger.info(f"Will use {name} from {highest_priority_source[0]} (priority {highest_priority_source[1]})")
        ...
        if len(priorities) > 1:
            ...
            for tool_class in tool_classes:
                if tool_class.__class__.__name__ == highest_class_name:
                    ...
                    logger.info(f"Registered tool '{name}' from {highest_class_name} (priority choice)")
                    break
