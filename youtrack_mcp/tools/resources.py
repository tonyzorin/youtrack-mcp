...
class ResourcesTools:
            ...
            try:
                ...
                for project in projects:
                    ...
                    if project_id and project_name:
                        resources.append({
                            "uri": URI_TEMPLATES["project"].format(project_id=project_id),
    ...
    def read_resource(self, uri: str) -> str:
        ...
        try:
            ...
            if parsed.scheme != YOUTRACK_URI_SCHEME:
                return json.dumps({"error": f"Invalid URI scheme: {parsed.scheme}. Expected: {YOUTRACK_URI_SCHEME}"})
            ...
            logger.debug(f"Parsed URI: scheme={parsed.scheme}, path={path}, query={query_params}")
                ...
                if path[0] == "projects" and path[2] == "issues":
                    ...
                    try:
                        ...
                        try:
                            ...
                            return json.dumps({
                                "contents": [{
                                    "uri": URI_TEMPLATES["issue_comments"].format(issue_id=issue_id),
                                    "mimeType": "application/json",
                                    "text": json.dumps(comments)
                                }]
                            })
                        except Exception as direct_error:
                            logger.error(f"Error with direct API call for comments: {direct_error}")
                            raise
    ...
    def get_project_issues(self, project_id: str) -> str:
        ...
        try:
            ...
            return json.dumps({
                "contents": [{
                    "uri": URI_TEMPLATES["project_issues"].format(project_id=project_id),
                    "mimeType": "application/json",
        ...
        try:
            try:
                ...
                try:
                    ...
                    if 'id' not in issue_data and '$type' in issue_data and issue_data.get('$type') == 'Issue':
    ...
    def get_issue_comments(self, issue_id: str) -> str:
        ...
        try:
            ...
            try:
                ...
                comments_with_details = self.client.get(f"issues/{issue_id}/comments?fields=id,text,created,updated,author(id,login,name)")
                logger.debug(f"Retrieved {len(comments_with_details)} comments for issue: {issue_id}")
                ...
                logger.warning(f"Error retrieving detailed comments with original API: {api_error}")
